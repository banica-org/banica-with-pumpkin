...
class AuroraServiceImplTest {
    ...
    void subscribeForItemShouldVerifyMethodCalls() {
        ...
        verify(marketSubscriptionServiceImpl, times(1)).subscribe(AURORA_REQUEST_BANICA, subscriber);
        verify(marketState, times(1)).generateMarketTicks("banica");
        verify(marketSubscriptionServiceImpl,times(1)).convertTickResponseToAuroraResponse(tick1);
        verify(marketSubscriptionServiceImpl,times(1)).convertTickResponseToAuroraResponse(tick2);
